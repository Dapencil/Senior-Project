...
public:
                               ...
                               const Modulus &plain_modulus)
    {
        ...
        for (size_t i = 0; i < rowCnt; i++)
        {
            ...
            encoder_.encode(vector<uint64_t>(batch_size_, transformDoubleToUInt(biases_[i], 32, plain_modulus)), encoded_bias);
...
public:
    ...
    vector<Ciphertext> predict(const vector<Ciphertext> &input)
    {
        ...
        for (auto &layer : layers_)
        {
            ...
            current_input = layer.forward(current_input, encoder_, evaluator_, relin_keys_, plain_modulus_);
...
int main()
{
    ...
    size_t scale = 32;
    vector<vector<double>> inputs = readBinaryInputFile("../test_iris_minmax.bin", feature_dim, batch_size);

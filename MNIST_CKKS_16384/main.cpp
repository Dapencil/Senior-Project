...
int main()
{
    ...
    vector<vector<double>> inputs = readBinaryInputFile("../test_nonpre_1.bin", 784, batch_size);
    vector<vector<double>> inputs_2 = readBinaryInputFile("../test_nonpre_2.bin", 784, 10000 - batch_size);
    vector<Ciphertext> encrypted_inputs, encrypted_inputs_2;
    ...
    for (size_t i = 0; i < inputs_2.size(); ++i)
    {
        encoder.encode(inputs_2[i], scale, encoded_row);
        encryptor.encrypt(encoded_row, encrypted_row);
        encrypted_inputs.push_back(encrypted_row);
    }
    ...
    vector<Ciphertext> result_vector_2 = model.predict(encrypted_inputs_2);
    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::seconds>(end - start);
    ...
    // writePredictionFile("../pred_ckks_16381_2.bin", pred);

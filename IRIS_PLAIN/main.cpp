...
int main()
{
    ...
    Dense layer_3 = Dense(weight_square_4, bias_square_4, batch_size, false);
    ...
    cout << "Model Execution time: " << duration.count() << " miliseconds" << endl;

    vector<int> pred = argmax_vertical(result, batch_size);
    printVector(pred, batch_size);
    writePredictionFile("../pred_plain_iris.bin", pred);
